...
import { ProductionMonitoringSetup } from './ProductionMonitoringSetup.js';
import { AutomatedMaintenanceSystem } from './AutomatedMaintenanceSystem.js';
...
export class ProductionInstaller extends EventEmitter {
     ...
     */
    async executeProductionInstallation(config) {
    ...
    generateMaintenanceConfig(config) {
        return {
            deploymentPath: config.deploymentPath,
            schedules: [
                {
    ...
    async setupUserOnboarding(config) {
...
`;
    ...
    async performFinalValidation(config) {
    ...
    generatePostInstallationSteps(config, result) {
        const steps = [
    ...
    generateSupportInformation(config) {
        return {
    ...
    displayInstallationSummary(config, result) {
...
//# sourceMappingURL=ProductionInstaller.js.map