...
class FLCTrainingContentGenerator {
    generateExecutiveTrainingModule() {
        return {
            moduleId: 'EXEC-001',
    ...
    generateProjectManagerTrainingModule() {
        return {
            moduleId: 'PM-001',
    ...
    generateCADOperatorTrainingModule() {
        return {
            moduleId: 'CAD-001',
...
export class UserTrainingFramework extends EventEmitter {
     ...
     */
    async generateFLCTrainingProgram(trainingPath) {
        ...
        await writeFile(join(trainingPath, 'training-configuration.json'), JSON.stringify(trainingConfig, null, 2));
    ...
    async generateHandsOnScenarios() {
        return [
            {
    ...
    async generateTrainingMaterials(config) {
        ...
        for (const module of config.trainingModules) {
            ...
            await writeFile(join(config.trainingPath, 'modules', `${module.moduleId}.md`), moduleContent);
        }
        // Generate quick reference guide
        const quickReference = this.generateQuickReferenceGuide(config);
        await writeFile(join(config.trainingPath, 'resources', 'quick-reference.md'), quickReference);
        console.log(chalk.green('✅ Training materials generated successfully'));
    }
    generateModuleMaterial(module) {
        return `# ${module.title}
...
`;
    ...
    generateQuickReferenceGuide(config) {
...
`;
     ...
     */
    updateUserProgress(userId, moduleId, completed, score) {
    ...
    getTrainingStatus() {
        return {
            ...
            activeUsers: Array.from(this.trainingProgress.values()).filter(u => Date.now() - u.lastActivity.getTime() < 7 * 24 * 60 * 60 * 1000 // Active in last 7 days
            ).length,
...
//# sourceMappingURL=UserTrainingFramework.js.map