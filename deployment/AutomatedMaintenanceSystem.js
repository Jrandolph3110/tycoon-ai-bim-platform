...
class LogRotationManager {
    async rotateAndCleanupLogs(deploymentPath, retentionDays) {
    ...
    async archiveOldLogs(logsPath, logFiles, cutoffDate) {
        const archivedFiles = [];
    ...
    async cleanupOldArchives(logsPath, cutoffDate) {
        const deletedFiles = [];
        const archivePath = join(logsPath, 'archive');
        try {
    ...
    async compressLargeLogs(logsPath) {
...
class PerformanceOptimizer {
    async optimizeSystemPerformance(deploymentPath) {
        const startTime = new Date();
        const warnings = [];
        const errors = [];
    ...
    async clearTemporaryFiles(deploymentPath) {
        let filesDeleted = 0;
        let spaceFreed = 0;
        const tempPaths = [
    ...
    async optimizeCache(deploymentPath) {
    ...
    async updatePerformanceConfigurations(deploymentPath) {
...
class SystemHealthMonitor {
    async performHealthCheck(deploymentPath) {
    ...
    async checkMCPServer(deploymentPath) {
        try {
            const serverPath = join(deploymentPath, 'src', 'mcp-server', 'dist', 'TycoonServer.js');
            await stat(serverPath);
            // Test server startup
            const { stdout } = await execAsync(`node TycoonServer.js --health-check`, { cwd: dirname(serverPath), timeout: 5000 });
            return {
                name: 'MCP Server',
    ...
    async checkAIComponents(deploymentPath) {
        try {
    ...
    async checkPerformanceMetrics(deploymentPath) {
...
export class AutomatedMaintenanceSystem extends EventEmitter {
     ...
     */
    async initializeMaintenanceSystem() {
    ...
    async executeMaintenanceSchedule(schedule) {
        const results = [];
    ...
    async executeMaintenanceTask(task) {
        switch (task.type) {
            case 'cleanup':
                return await this.logRotationManager.rotateAndCleanupLogs(this.config.deploymentPath, this.config.logRetentionDays);
            case 'optimization':
                return await this.performanceOptimizer.optimizeSystemPerformance(this.config.deploymentPath);
            case 'health_check':
    ...
    async logMaintenanceResults(scheduleName, results) {
    ...
    async performSelfHealing(health) {
            ...
            try {
                switch (component.name) {
                    ...
                    case 'System Resources':
                        ...
                        await this.logRotationManager.rotateAndCleanupLogs(this.config.deploymentPath, this.config.logRetentionDays);
...
//# sourceMappingURL=AutomatedMaintenanceSystem.js.map